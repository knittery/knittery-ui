...
class KnittingCarriageSpec extends Specification {
  ...
  "K-KnittigCarriage without pattern" should {
    "knit plain red stitches with one yarn and needles to B" in new K {
      val KnittingCarriageResult(needles, _, _, knitted) = carriageK(ToRight, allBWithRed, allA).check
      knitted.all must_== (1 to Needle.count).map(_ => PlainStitch(red))
    }
    "knit plain red stitches with one yarn and needles to D" in new K {
      val KnittingCarriageResult(needles, _, _, knitted) = carriageK(ToRight, allDWithRed, allA).check
      knitted.all must_== (1 to Needle.count).map(_ => PlainStitch(red))
    }
    "knit plain red stitches with one yarn and needles to E" in new K {
      val KnittingCarriageResult(needles, _, _, knitted) = carriageK(ToRight, allEWithRed, allA).check
      knitted.all must_== (1 to Needle.count).map(_ => PlainStitch(red))
    }
  ...
  "K-KnittigCarriage on KC2 with plain" should {
    "knit plain red stitches with one yarn and needles to B" in new K {
      val KnittingCarriageResult(needles, _, _, knitted) = carriageKPlain(evenOddPattern)(ToRight, allBWithRed, allA).check
    ...
    "move every needle to B and have red on the needles in the all B-pattern after even/odd" in new K {
      val KnittingCarriageResult(needles, _, _, knitted) = carriageKPlain(allBPattern)(ToRight, allBDEvenOddWithRedGreen, allA).check
    ...
    "move every needle to D and have red/green on the needles in the all D-pattern after even/odd" in new K {
      val KnittingCarriageResult(needles, _, _, knitted) = carriageKPlain(allDPattern)(ToRight, allBDEvenOddWithRedGreen, allA).check
    ...
    "knit plain red/green stitch pattern with even odd needles and even odd pattern" in new K {
      val KnittingCarriageResult(needles, _, _, knitted) = carriageKPlain(evenOddPattern)(ToRight, allBDEvenOddWithRedGreen, allA).check
  ...
  "K-KnittigCarriage on KC2 with MC" should {
    "knit plain red stitches with one yarn and needles to B" in new K {
      val KnittingCarriageResult(needles, _, _, knitted) = carriageKMC(evenOddPattern)(ToRight, allBWithRed, allA).check
      knitted.all must_== (0 until Needle.count).map(_ => PlainStitch(red))
    }
    "move every second needle to D in the even odd pattern" in new K {
      val KnittingCarriageResult(needles, _, _, knitted) = carriageKMC(evenOddPattern)(ToRight, allBWithRed, allA).check
    ...
    "move every needle to B and have red/green on the needles in the all B-pattern after even/odd" in new K {
      val KnittingCarriageResult(needles, _, _, knitted) = carriageKMC(allBPattern)(ToRight, allBDEvenOddWithRedGreen, allA).check
      needles.all.zipWithIndex.forall {
        case (NeedleState(pos, yarn), index) =>
          pos must_== NeedleB
    ...
    "move every needle to D and have red/green on the needles in the all D-pattern after even/odd" in new K {
      val KnittingCarriageResult(needles, _, _, knitted) = carriageKMC(allDPattern)(ToRight, allBDEvenOddWithRedGreen, allA).check
      needles.all.zipWithIndex.forall {
        case (NeedleState(pos, yarn), index) =>
          pos must_== NeedleD
          yarn.map(_.yarn) must_== (if (index % 2 == 0) Set(red) else Set(green))
    ...
    "knit plain red/green stitch pattern with even odd needles and even odd pattern" in new K {
      val KnittingCarriageResult(needles, _, _, knitted) = carriageKMC(evenOddPattern)(ToRight, allBDEvenOddWithRedGreen, allA).check
      knitted.all.zipWithIndex.foreach {
        case (stitch, index) if index % 2 == 0 => stitch must_== PlainStitch(red)
        case (stitch, index) => stitch must_== PlainStitch(green)
      }
