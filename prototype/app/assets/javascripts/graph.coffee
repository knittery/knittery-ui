###
  Graphing library with (spring) layouting.
###
define(["threejs"], (THREE) ->
      ...
      for node in @graph.nodes
        ...
        if not cluster?
          clusters[i] = {
            ...
            y: y
            z: z
            centerOfMass: new Vector(min.x + sizePart.x * (x + 0.5), min.y + sizePart.y * (y + 0.5),
                min.z + sizePart.z * (z + 0.5))
            mass: 1
            nodes: [node]
          }
        else
    ...
    step: ->
      ...
      for clusterA, i in clusters when clusterA?
        for j in [i..clusters.length - 1]
          clusterB = clusters[j]
          if clusterB?
            dx = clusterA.x - clusterB.x
            dy = clusterA.y - clusterB.y
            dz = clusterA.z - clusterB.z
  ...
  result =
    Graph: Graph
    ClusterSpringLayout: ClusterSpringLayout
