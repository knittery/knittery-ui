...
object FormGiving {
  ...
  def raglanDecrease(bed: Bed, at: LeftRight): PlannerM[Needle] = for {
    needle <- Planner.preconditions { s =>
  ...
  def raglanWithLCarriage(depth: Int, left: Boolean = true, right: Boolean = true): Planner = for {
    _ <- Planner.preconditions(s => require(s.workingNeedles(MainBed).size >= depth * 2, "Not enough working needles"))
    ...
    _ <- (depth - 1 to 0 by -1).toVector.traverse { offset =>
      val leftPattern: NeedleActionRow =
        if (left) n => if (n == first - offset) NeedleToD else NeedleToB
        else n => NeedleToB
      val rightPattern: NeedleActionRow =
        if (right) n => if (n == last + offset) NeedleToD else NeedleToB
        else n => NeedleToB
