...
object FormGiving {
  ...
  def raglanDecrease(at: Needle, direction: Direction): Planner = {
    val sign = if (direction == ToLeft) -1 else 1
    (0 until (sign * 3) by sign).map(at + _).reverse.toVector.
      traverse { n =>
        Planner.step(RetireNeedle(n, direction))
      } >> Planner.noop
  }

  /** Decrease one needle at the specified site (3-needle raglan). */
  def raglanDecrease(at: LeftRight): PlannerM[Needle] = for {
    ...
    _ <- raglanDecrease(needle, direction)
  } yield needle
}